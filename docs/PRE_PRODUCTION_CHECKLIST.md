...
| 7 | Churn re-scoring completes within **6-hour SLA** on **500-account** corpus | ☐ | `celery -A app.pipeline.celery_app call app.pipeline.tasks.batch_rescore_customers` on a DB with ≥500 customers; measure wall time vs `SCORING_CADENCE_HOURS=6`. |
...
| 9 | Database migrations **idempotent** and tested against production schema clone | ☑ Partial | `cd backend && alembic upgrade head` on fresh Postgres (CI runs this). For prod clone: restore snapshot → `alembic upgrade head` → confirm no errors. Chain: `001` → `002` → `003`. |

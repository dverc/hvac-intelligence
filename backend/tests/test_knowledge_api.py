...
@pytest.mark.asyncio
async def test_upload_and_list_document(auth_client, db_session):
    ...
    listing = await auth_client.get(f"/api/v1/knowledge/{org_id}/documents")
...
@pytest.mark.asyncio
async def test_delete_document_soft(auth_client, db_session):
    org_id = SEED_ORG_ID_STR
...
@pytest.mark.asyncio
async def test_create_service_catalog_via_knowledge_api(auth_client, db_session):
    org_id = SEED_ORG_ID_STR
...
@pytest.mark.asyncio
async def test_patch_service_catalog_updates_price(auth_client, db_session):

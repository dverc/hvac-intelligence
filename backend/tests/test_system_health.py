...
@pytest.mark.asyncio
async def test_system_health_returns_200(auth_client):
...
@pytest.mark.asyncio
async def test_system_health_healthy_when_db_ok(auth_client):
    response = await auth_client.get("/api/v1/system/health")
    body = response.json()
...
@pytest.mark.asyncio
async def test_system_health_degraded_when_redis_slow(auth_client):
...
@pytest.mark.asyncio
async def test_system_health_metrics_counts(auth_client, seeded_customer):
    del seeded_customer
    response = await auth_client.get("/api/v1/system/health")
